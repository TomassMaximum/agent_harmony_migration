...
1. **会话持久化**：将会话消息（self.messages）保存到持久化存储中，支持恢复。
2. **聊天管理**：组织多个会话（session）为一个聊天（chat），便于用户管理长期任务。
3. **记忆摘要**：自动生成会话摘要，减少上下文长度，提高长期记忆效率。
4. **轻量级实现**：使用JSON文件存储，无需额外数据库依赖。
5. **向后兼容**：不影响现有AgentLoop和Web接口的功能。
...
只需实现新的存储类，保持与 `SessionMemory` 和 `ChatMemory` 相同的接口即可。

### 3. 清理策略

- 基于时间的过期删除（如30天未活跃）。
- 基于大小的限制（如最多保留100个聊天或1000个会话）。
- 可配置的清理脚本。

### 4. 记忆检索增强

- 基于向量嵌入的语义搜索，快速定位相关历史会话。
- 关键词索引。
...
本记忆模块以两层结构（聊天和会话）为 Agent 提供了强大的记忆功能，确保了对话的连续性、可恢复性以及跨会话的上下文管理，为长期协作任务提供了基础支持。
